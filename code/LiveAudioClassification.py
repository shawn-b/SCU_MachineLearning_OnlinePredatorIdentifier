...
device_id = -1          # audio device identifier
...
try:
    with sr.Microphone(device_index=device_id, sample_rate=sample_rate, chunk_size=chunk_size) as src:
        ...
        print("Say 'stop' to terminate program.\n")
        try:
            while True:
                ...
                try:
                    start_time_w_sr = time.time()
                    text = r.recognize_google(audio)
                    start_time_wo_sr = time.time()
                ...
                end_time = time.time()
                print("Execution time (with sr): %s seconds" % (end_time - start_time_w_sr))
                print("Execution time (w/o sr): %s seconds" % (end_time - start_time_wo_sr))
                print()
